function [xmean,stats] = reconstruct_track_v2(t,p,h,s,tF,XF,q,sigmaFall,nuFall,tsurf,nburnin,nsamples,thin,initsigma,workingDir,doParallel,plots)
...
%
% Version 2. Model structure with observation model for FPGS position fixes
%
% Output arguments:
...
tseg = unique(tF); % start time of track segment j
...
% Indices for nested indexing in observation model
% (only really needed if >1 logger is used simultaneously)
NF = length(tF);
ind = nan(NF,1);
for i=1:NF
...
hold on; plot(XF(:,1),XF(:,2),'o','MarkerFaceColor','r','MarkerEdgeColor','none','Linewidth',0.5); hold off
legend('Most probable track','Fastloc-GPS position fix')
...
%% Nested functions
    function writeModel(workingDir)
        modelstr = {'model';'{';[];'## Priors';'sigma[1] ~ dunif(0, 0.1)  # Process noise sd, x';'sigma[2] ~ dunif(0, 0.1)  # Process noise sd, y';[];...
            ...
            '## Priors on 1st position';'x[1,1] ~ dt(0, tauF[1,1], nuF[1,1])';'x[1,2] ~ dt(0, tauF[1,2], nuF[1,2])';[];...

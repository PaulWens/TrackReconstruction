function [xmean,stats] = reconstruct_track_v4(t,p,h,s,tb,Xw,m,sigmaw,nburnin,nsamples,thin,initsigma,workingDir,doParallel,plots)
...
%
% Version 4. Model structure with a simplified observation model for 
% whale positions (lat,lon) obtained from visual tracking. Thus ignores
% the effect of range on observation error.
%
% Output arguments:
...
% Note(2)-All position vectors are in Cartesian coordinates in km relative to the coordinates of the first position fix on the animal
...
%% Nested functions
    function writeModel(workingDir)
        modelstr = {'data {';'tauw[1] <- sigmaw[1]^(-2)';'tauw[2] <- sigmaw[2]^(-2)';'firsttau <- ifelse(m[1]-1, tauw[2], tauw[1]) # precision of first location';'}';[];...
            ...
            '## Priors on 1st position ';'x[1,1] ~ dnorm(0, firsttau)';'x[1,2] ~ dnorm(0, firsttau)';[];...
            ...
            '## Observation model for visual position data';'for(j in 1:N){';'tauw1[j] <- ifelse(m[j]-1, tauw[2], tauw[1])';'Xwx[j] ~ dnorm(x[j,1], tauw1[j])';'Xwy[j] ~ dnorm(x[j,2], tauw1[j])';'}';'}'};
        mkdir(workingDir)
